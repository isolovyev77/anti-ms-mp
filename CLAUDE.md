...
**Где он физически:** ЖИВОЙ источник — только на Vultr (`/opt/anti-ms-mp-watcher/memory.md`),
единый, чтобы не было рассинхрона. Все пишут туда (оркестратор локально, ноутбук по SSH).
В репозитории — периодический СНИМОК `backup/memory.md` (для долговечности и чтения офлайн;
НЕ писать в него — он только для чтения/бэкапа, перезаписывается снимком с Vultr).
`CLAUDE.md` лежит и в репо (для сессий), и на Vultr `/opt/anti-ms-mp-watcher/CLAUDE.md`
(чтобы headless-Claude авто-починки на Vultr тоже видел правила).
...
- **Планирование**: задачи с прод/необратимыми изменениями (серверы VDSina/Vultr, миграции БД,
  sudoers/права, деплои, удаление данных) — оформлять через **plan mode** (план в панели + Approve
  ДО действий). Мелкие обратимые правки (локальный код в репо, документация) — план текстом + сразу.
