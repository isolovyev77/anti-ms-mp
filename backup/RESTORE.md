...
| Дашборд | Vercel `anti-ms-mp.vercel.app` ← репозиторий (корень → каталог) | `anti-ms-dashboard/protos/proto_a_dark.html` (тёмный командный центр) |
...
# залить из backup/vultr/: orchestrator.sh, healthcheck.py
# залить ~/anti-ms-mp-secrets/vultr.env → .env (chmod 600); проставить AUTOFIX=0/1
# положить копию ключа VDSina → vdsina_key (chmod 600) — для авто-починки
...
- Репозиторий деплоится как есть (корневой `index.html` = каталог, ссылки на
  `anti-ms-dashboard/`). Боевой дашборд — `anti-ms-dashboard/protos/proto_a_dark.html`.
