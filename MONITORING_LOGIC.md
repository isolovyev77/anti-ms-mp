...
### 🔎 8 поисковых запросов на мониторинге (= MON_QUERIES_DEFAULT, версия 3)
Набор отобран эмпирически: каждый кандидат прогонялся, считался % новых
`product_id` (которых не дают остальные запросы). Включены дающие ≥10% уникальных.

1. **Office ключ** — широкий базовый (ловит все годы 2013-2024 и редакции)
2. **Office активация** — альт. формулировка, +3-5%
3. **Microsoft 365** — подписки Personal/Family/Business, +17 уникальных
4. **Office LTSC** — корпоративные бессрочные Pro Plus / Standard LTSC
5. **Office 2024** — годовой якорь
6. **Office 2019** — годовой якорь
7. **Майкрософт Офис 2021** — кириллица+год, +15% новых (другой срез продавцов)
8. **Майкрософт Офис 2016** — кириллица+год, +24% новых (под 2016 нет лат. якоря)

**Отклонены (НЕ добавлять — проверено):**
- «Microsoft Office ключ активации», «MS Office ключ активации»,
  «Office 2021 ключ активации» — подмножества «Office ключ» (≤1 новая)
- «Office 2021 Home Business» — Home&Business ловятся широкими
- чистая кириллица без года («Майкрософт Офис ключ», «Офис активация») — 1-5%
  (маркетплейс матчит транслитерацию Office↔Офис)
- «Майкрософт Офис 2024/2019» — 0% новых (покрыты лат. Office 2024/2019 + МО 2021)

**Правило для будущих кандидатов:** прежде чем добавить — прогнать на
ozon+yandex+wildberries (3 стр), применить `title_ok`, посчитать долю
`product_id`, которых нет в текущей выборке. Порог ≈ ≥10% новых, иначе
подмножество. Кириллица+год полезна только там, где нет латинского якоря.

**⚠️ Синхронизация:** этот набор продублирован в:
- `anti-ms-dashboard/index.html` → `MON_QUERIES_DEFAULT` (+ блок «🔎 Поисковые запросы»)
- `cloakbrowser-lab/scrape_{ozon,avito,wb,ym}_cloak.py` → `DEFAULT_QUERIES` / argparse default
- `scraper/scrape-marketplaces.js` → `CONFIG.queries`

При изменении набора править ВО ВСЕХ местах + поднять `MON_QUERIES_VERSION`.
Рассинхрон = дашборд показывает один набор, данные собраны по другому → фильтр
по query даёт 0 карточек.

**⚠️ title_ok и кириллица:** для запросов «Майкрософт Офис N» карточки бывают
с чисто кириллическим названием. `title_ok` принимает `'офис' + ('microsoft' ИЛИ
'майкрософт')` и `'365' + ('microsoft' ИЛИ 'майкрософт')` — иначе кириллические
карточки отфильтровываются.

Хранятся в localStorage, версионируются через `MON_QUERIES_VERSION=3`
(при смене дефолта старая версия у пользователя сбрасывается).
...
сохраняется в `localStorage`. Версионируется через `MON_QUERIES_VERSION=3`
(при апгрейде дефолта старый localStorage сбрасывается).

`MON_QUERIES_DEFAULT` (8 запросов, подробности и обоснование — в TL;DR выше):
Office ключ · Office активация · Microsoft 365 · Office LTSC · Office 2024 ·
Office 2019 · Майкрософт Офис 2021 · Майкрософт Офис 2016
