...
## 12. История изменений (последние)

- `a988fea` — Мультиключевая сортировка (disc↓, price↑, title↑)
- `da17f84` — Уточнённые `official_price()` (7 типов) + фильтр `price >= 0.5×op`
- `7b71492` — CSV-экспорт корректно экранирует запятые/кавычки (RFC 4180)
- `b050c66` — Фикс NBSP в WB DOM-парсере (`2 021 ₽` → `2021` вместо `21`)
- `1cac745` — Полная замена снапшота через CloakBrowser+VDSina (все 4 площадки)
- `19a7b6e` — Таб «Мониторинг» первым, АПКИТ в списке ассоциаций, favicon
