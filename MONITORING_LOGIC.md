...
### 💳 Признак `apay` — оплата через инфраструктуру Avito (юридически важно)
Avito заявляет, что является доской объявлений и несёт ответственность только
за объявления **с оплатой через её инфраструктуру** (Авито Доставка / безопасная
сделка). Поле `apay` помечает такие карточки → прямая зона ответственности Avito.

- **Детектор** (`cloakbrowser-lab/enrich_avito_pay.py`): открывает страницу
  каждого Avito-объявления, ищет **реальную кнопку покупки**: `Купить с доставкой`,
  `Заказать с доставкой`, `Добавить в корзину`, `Купить сейчас`, `Оформить заказ`.
  Если только `Написать` / `Показать телефон` → `apay=false` (оплата вне Avito).
- **ВАЖНО — не путать с приглашением продавцу**: текст «Отправите Авито Доставкой?»
  есть почти на всех карточках — это маркетинговое предложение продавцу, НЕ признак
  активной оплаты. Детектор его игнорирует.
- Поле `apay` хранится в `cloakbrowser-lab/avito_pay.json` (`{id: true/false}`),
  подмешивается в MON_RAW через `import_cloak_full.py` (поле `apay:true` только
  для true-карточек, чтобы не раздувать).
- **UI**: красный бейдж «💳 оплата через Avito» под меткой площадки + фильтр-кнопка
  «💳 Только через Avito» + колонка «Оплата через Avito» в CSV/XLSX.
- **Особенность парсинга**: enrichment требует захода на КАЖДУЮ страницу товара —
  Avito банит после ~220 запросов подряд. Скрипт делает паузы 2.5-5 сек,
  cooldown 60 сек при 5 блокировках, поддерживает возобновление (None
  перепроверяется при следующем запуске).
- Доля контрафактных Office-карточек с `apay=true` на Avito: **~15-40%**
  (зависит от выборки; первые/коммерческие карточки чаще с оплатой).
- **📄 Полное правило детекции — в `scraper/AVITO_PAY_DETECTION.md`**
  (точные селекторы, что НЕ путать, антибан, формат данных, интеграция в Supabase).
  Рабочий скрипт: `scraper/enrich_avito_pay.py`.
